"""
Sensor Data Collector - Standalone Mode (Raspberry Pi)
======================================================

DEPLOYMENT: Raspberry Pi (Sensor System)
MODE: Standalone/Interactive

This script runs directly on the Raspberry Pi with attached sensors:
- Thermal camera (MLX90640 or Lepton35)
- ToF distance sensor (TOF400F)
- Raspberry Pi camera module

USAGE:
    Run directly on Raspberry Pi for interactive data collection:

    python sensor_data_collector.py --object <object_name> --captures <num>

FEATURES:
    - Interactive manual control via command line
    - Direct sensor access and data collection
    - Local video recording and photo capture
    - Does NOT require network communication with control PC
    - Suitable for standalone sensor testing and manual data collection

NETWORK: Not required (operates independently)

See sensor_collector_server_raspi.py for networked operation with arm control.
"""
...
class DataCollector:
    ...
    TEMP_THRESHOLDS = {
        ...
        "max_valid": 200.0,  # Celsius
        "max_std_dev": 50.0,  # Max standard deviation as a validity check
    }
    ...
    def _check_disk_space(self, min_space_mb=500):  # Increased for video recording
        ...
        try:
            if not self.output_dir:
                raise OSError("Output directory path not specified")
            ...
            elif free_mb < min_space_mb * 2:
                logger.warning(f"Low disk space warning: Only {free_mb:.1f}MB available")

            logger.info(f"Available disk space: {free_mb:.1f}MB")

        except OSError as e:
            raise OSError(f"Error checking disk space: {str(e)}") from e
    ...
    def _validate_temperature_data(self, temp_data):
        ...
        if debug_thermal:
            ...
            std_dev: floating[Any] | Any = np.std(data_array)
    ...
    def _convert_h264_to_mp4(self, h264_path):
        ...
        try:
            ...
            # Run FFFmpeg to convert the file
            ...
            if result.returncode == 0:
                ...
                # Check if an original file should be deleted
    ...
    def record_video(self, duration=None):
        ...
        Args:
            duration (int, optional): Recording duration in seconds. If None, the user must stop manually.
        ...
        try:
            ...
            # Add a small delay before starting recording to ensure the camera
            # is ready
            time.sleep(0.5)
            ...
            if duration is None:
                ...
                # Wait for duration and a small buffer time
                actual_duration = duration + 2  # Adding 2-second buffer
    ...
    def collect_data(self, capture_index, retry_count=3):
        ...
        if self.camera:
            try:
                ...
                # Capture photo with an explicit path (disable enhancements to
                # preserve colors)
        ...
        while attempts < retry_count and not (thermal_success and tof_success):
            ...
            try:
                ...
                if not tof_success and self.tof_sensor:
                    ...
                    # Record TOF distance, even if invalid (may be useful for
                    # debugging)
                    tof_metadata["tof_distance_mm"] = self.last_tof_distance
    ...
    def __del__(self):
        """Cleanup method to ensure a proper sensor shutdown"""
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(description="Sensor Data Collection Tool")
    ...
    parser.add_argument("-c", "--captures", type=int, default=None, help="Number of captures")
    parser.add_argument(
        "-output",
        "--output",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default="/dev/ttyAMA0",
        help="TOF sensor serial port (default: /dev/ttyAMA0)",
    )
    parser.add_argument(
        "-C",
        "--camera",
        type=str,
        default="Lepton35",
        choices=["MLX90640", "Lepton35"],
        help="Thermal camera type (default: MLX90640)",
    )
    parser.add_argument("-r", "--resume", action="store_true", help="Resume a previous scan")
    parser.add_argument(
        "-s",
        "--start-from",
        type=int,
        default=1,
        help="Position to start from (used with --resume, default: 1)",
    )
...
def main():
    ...
    try:
        ...
        collector = DataCollector(
            ...
            tof_port=args.tof_port,
            camera_type=args.camera,
        )

        # Determine starting position
        start_position = args.start_from if args.resume else 1

        # Run data collection
        collector.run_collection(args.captures, start_from=start_position)
